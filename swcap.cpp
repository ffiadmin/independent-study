...
extern "C" void FORTRANNAME(wqromb)( double *c, int *rpow, double *alp, double *answer);                     
...
//              IF mehtod is CAP form the Wmat for the CAP
       	...
       	double Wmat[nbasis][nbasis];
	double Wcore[nbasis][nbasis],Wcore1[nbasis][nbasis];
               
	for (int row=0; row<nbasis; row++){
		for (int col=0; col<nbasis; col++){

  			double gamma_sum = alpha[row]+alpha[col];
			Wmat[row][col]=norm[row]*norm[col]*Wcalc(gamma_sum);
                        cout << norm[row] << "  " <<norm[col] << endl;
