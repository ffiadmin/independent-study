...
c     **** uses standard intgral formulas for gto's
     ...
     + nbasis,nnuc,nthet,naux,info,kk,nalp,icnt,
     + iicnt
...
c               write(6,*)'in theta loop',theta,scale,eigval(iii)
                iicnt=iicnt+1
                if(iicnt.eq.1)then
                    icnt=icnt+1
                endif
...
99    continue
     ...
     +    (eta(i+1)-eta(i))
	  ...
	  if (dabs(dermag).lt.dabs(dermin)) then
	      dermin=dermag
	      etamin=eta(i)
	      valmin=val(i)
	      angmin=ang(i)
c     write(6,*)'made it in R loop,alpreal,angmin,etamin,valmin,dermin'
c     write(6,*)alpreal,angmin,etamin,valmin,dermin
	  endif
999   continue

c     write(6,*)'min for R   ',alpreal,angmin,etamin,valmin,dermin


          if (dabs(dermin).lt.dabs(derold))then 
                derold=dermin
                 oldalp =alpreal
                 oldthet=angmin
                 oldeta=etamin
...
888   continue          
         ...
         if (derold.lt.derref) then
               if (istep.lt.nalp)then
                  alpstp=alpstp/dfloat(nalp-1)
                  thstep=thstep/dfloat(nthet-1)
               endif
            alpstrt = oldalp -alpstp*dfloat(nalp-1)/2.0d+00
            thstart=oldthet -thstep*dfloat(nthet-1)/2.0d+00
            derref=derold
      ...
      write(6,*)'after',oldalp,oldthet,oldeta,oldval,derold
