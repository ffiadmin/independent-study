...
c     **** uses standard intgral formulas for gto's
     ...
     + PI,t1,t2,t3,t4,norm(maxbas),min,max

      double precision ovrlp,r1
      double precision kenergy

      real*8     smat(maxbas,maxbas),
     ...
     + dermag,dermin,ang(maxstp),angmin
      ...
      complex*16 scale,vint(maxbas,maxbas),
...
70    continue
     ...
     +   VR,maxbas,work,maxbs3,rwork,info)
      ...
      do 86 iii = 1,nbasis
            ...
            write(6,*)theta,scale,eigval(iii)
            eta(nscan)=scale
...
86    continue
      ...
      call zgemm('N','N',nbasis,nbasis,nbasis,XX1,VR,maxbas,
     +   xmatx,maxbas,XX2,xtmp,maxbas)

      call zgemm('N','N',nbasis,nbasis,nbasis,XX1,xmat,maxbas,
     +   xtmp,maxbas,XX2,VR,maxbas)
...
c     write(6,*)l1,l2,wx,wy,wz     
      ...
      REAL*8 FUNCTION OL1D(l1,l2,PAx,PAb,gamma)
...
c     write(6,*)'a,sum',a,sum
     ...
     +           nx,ny,nz,venergy,scale)
      ...
      complex*16 venergy,sum,rcp2,c7

      complex*16 c1,c2,c3,gtest,c4,scale,
     + cx,cy,cz
...
c     write(6,*)'i,r,u,t1',i,j,k,t1
      ...
      Complex*16 FUNCTION gtest(a,x)
      complex*16 x, sum,c1,c2,c3,sum2,xx2,cdig,b
      real*8 a,r1,r2,sumnrm
      integer n,q,fct
