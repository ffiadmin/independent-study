...
// 			THis is a call to a generic model potential numerical integration routine.
                        ...
                        Mint[row][col]= norm[row]*norm[col]* Model_pot(l[row],l[col],m[row],m[col],n[row],n[col],
                                                 alpha[row],alpha[col]);
...
static double Model_pot(int l1, int l2, int m1, int m2, int n1, int n2, double alp1, double alp2){
	...
	double xterm1, xterm2, answer;
	double yterm1, yterm2;
	double zterm1, zterm2,t1;
	...
	double rpower2 = 0.0;
  	...
  	FORTRANNAME(qromb)(&la,&alp,&answer);                     
	xterm1=answer;
	FORTRANNAME(qromb)(&lb,&alp,&answer);                     
	xterm2=answer;

	FORTRANNAME(qromb)(&ma,&alp,&answer);                     
	yterm1=answer;
	FORTRANNAME(qromb)(&mb,&alp,&answer);                     
	yterm2=answer;

	FORTRANNAME(qromb)(&na,&alp,&answer);                     
	zterm1=answer;
	FORTRANNAME(qromb)(&nb,&alp,&answer);                     
	zterm2=answer;

        t1 = xterm2*yterm1*zterm1 +
             xterm1*yterm2*zterm1 +
             xterm1*yterm1*zterm2;
        t1= 8.0*t1; 
        return t1;  
}
