...
int main(){
	...
	double Tint[nbasis][nbasis],Smat[nbasis][nbasis],Vint[nbasis][nbasis];
  	...
  	for(int ii=0; ii<nbasis; ii++){
            in_stream >> l[ii] >> m[ii] >> n[ii] >> alpha[ii] >> x[ii] >> y[ii] >> z[ii];
            cout << alpha[ii];
        }
        /*  
	...
	nx[1]=0.0;
	ny[1]=0.0;
	nz[1]=3.9/0.529177249;
	nchg[1]=0.0;  
	nx[2]=0.0;
	ny[2]=0.0;
	nz[2]=-3.9/0.529177249;
	nchg[2]=0.0;  
		...
		for (int col=0; col<nbasis; col++){
	       ...
	       for (int center=0; center<nnuc; center ++){
			Vint[row][col]= Vint[row][col] + nchg[center]*
                ...
                cout << Fmat1[0][0] << "1,1" <<endl;
                cout << Fmat1[0][1] << "1,2" <<endl;
                cout << Fmat1[1][0] << "2,1" <<endl;
                cout << Fmat1[1][1] << "2,2" <<endl;

                FORTRANNAME(dsyev)("V", "U", &nbasis, *Fmat1, &nbasis,  fvec, aux, &naux, &info);	

                cout << fvec[0] << "first" << endl;
                cout << fvec[1] << "second" << endl; 
*/
}       
