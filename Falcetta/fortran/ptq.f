...
c     **** uses standard intgral formulas for gto's
      ...
      double precision alpha(maxbas),aref(maxbas),
     + x(maxbas),y(maxbas),z(maxbas),BSCALE
     ...
     + dermag,dermin,ang(maxstp),angmin,qrad,
     + rrr,rrrval,radpsi,value(1000),totalval,vreal,Q,
     + qslope
...
c     note: all coordinates are read in in angstroms and then converted to au before printing
      ...
      do 10 i = 1, nbasis
           ...
           x(i) = x(i)/0.529177249d+00
           y(i) = y(i)/0.529177249d+00
           z(i) = z(i)/0.529177249d+00
           aref(i)=alpha(i)
           write(6,*)l(i),m(i),n(i),x(i),y(i),z(i),alpha(i)
10    continue

      read(5,*)Q,qrad,qslope
      write(6,*)'the quadrupole moment is: ',Q   ,'  in atomic units.'
      write(6,*)'the cutoff radius is : ', qrad  ,'  in atomic units.'
      write(6,*)'the slope of repulsive potential is : ', qslope  ,
     +    '  in atomic units.'
      ...
      BSCALE=0.95d+00
      do 777 iijj=1,2
           BSCALE=BSCALE+0.05d+00
           do 776 i = 1,nbasis
             alpha(i)=aref(i)*BSCALE
776        continue
...
30    continue
     ...
     +           kenergy)
                ...
                tint(jc,ir)=dcmplx(r1,0.0d+00)


                call qromb(Q,alpha(ir),alpha(jc),
     +          norm(ir),norm(jc),qrad,r1,         
     +          qslope)
                vint(ir,jc)=dcmplx(r1,0.0d+00) 
                vint(jc,ir)=vint(ir,jc)
...
c          write(6,*)'scale ',scale
      ...
      do 40 ir=1,nbasis
       ...
       core(ir,jc)=tint(ir,jc)
     +  +vint(ir,jc)
       core(jc,ir)=core(ir,jc)


c               write(6,*) 'row, col, S ',ir,jc,smat(ir,jc)
c               write(6,*) 'row, col, T ',ir,jc,tint(ir,jc)
c               write(6,*) 'row, col, V ',ir,jc,vint(ir,jc)
     ...
     +   VR,maxbas,XX2,xtmp,maxbas)
      ...
      do 86 iii = 1,nbasis
            ...
            do 712 kstep=1,500    
                rrrval = rrrval+0.01
                rrr=rrrval*rrrval
                Crtmp = (0.00d+00,0.00d+00)
                do 713 kbas=1,nbasis
                    radpsi =rrrval*rrrval*norm(kbas)*
     +              dexp(-alpha(kbas)*rrr)
                    c111=dcmplx(radpsi,0.0d+00)
                    Crtmp=Crtmp + c111*xtmp(kbas,iii)
713             continue
            value(kstep) = rrr*dreal(dconjg(Crtmp)*Crtmp)
            totalval=totalval+value(kstep)
712         continue
            rrrval=0.0
            do 715 kstep=1,500
               rrrval=rrrval+0.01
               value(kstep)=value(kstep)*100.0/totalval
               write(6,*) rrrval,value(kstep)
715         continue
...
c
      ...
      subroutine qromb(Q,a1,a2,n1,n2,qrad,v,qslope)

      double precision a1,a2,qrad,g,r1,r2,n1,n2,r3,PI,
     + Q,qslope,r4,qmag,r5,a,b,ss,EPS,v
      INTEGER JMAX,JMAXP,K,KM,lpow,it,j
      PARAMETER ( a=0.0d+00,
...
c     c2=dcmplx(r2,0.0d+00)
      ...
      r3= n1*n2*PI*8.0d+00/105.0d+00
       if(r3.ge.1.0d+19)r3=1.0d+19
c     write(6,*)'n1,n2,r3',n1,n2,r3
      r4 = 1.0d+00/(2.0d+00*g*g)

      do 11 j=1,JMAX
        call trapzd(a,b,s(j),j,it,qmag,qslope,qrad,g)
        if (j.ge.K) then
          call polint(h(j-KM),s(j-KM),K,0.d+00,ss,dss)
          if (abs(dss).le.EPS*abs(ss)) go to 999
        endif
        s(j+1)=s(j)
        h(j+1)=0.25d+00*h(j)
11    continue
...
999   r3 = r3*(Q*r4 + ss)
      v =r3
c     write(6,*)'RESULT,g,ss,v',g,ss,v  

      return 
      end


      SUBROUTINE trapzd(a,b,s,n,it,qmag,qslope,qrad,g)
      INTEGER n
      double precision a,b,s,qmag,qslope,qrad,g,func
      EXTERNAL func
      INTEGER it,j
      double precision del,sum,tnm,x
      if (n.eq.1) then
        s=0.5d+00*(b-a)*(func(a,qmag,qrad,qslope,g)
     +                 + func(b,qmag,qrad,qslope,g))
      it=1
      else
        tnm=dfloat(it)
        del=(b-a)/tnm
        ...
        sum=0.0d+00
        do 11 j=1,it
          sum=sum+func(x,qmag,qrad,qslope,g)
          x=x+del
11      continue
...
13    continue
      ...
      double precision FUNCTION func(x,qmag,qrad,qslope,g)
C     integral of FUNC
      double precision x,qmag,qrad,qslope,g

      func =(x**6)*qmag*dexp(-g*x*x-qslope*(x-qrad))           
      END
